...
# The default for the `api_base` config parameter.
#
# Here rather than beside the uploader that spends most of its time talking to
# it: the version check needs the same URL and runs with no API key, and
# reaching into the collector for a constant would pull protobuf into a path
# that never encodes a signal.

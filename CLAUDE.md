...
Launcher argv policy: workload commands otherwise pass through byte-for-byte, with engine-specific metric adjustments: SGLang appends `--enable-metrics` (its Prometheus endpoint is off by default), vLLM strips `--disable-log-stats`, and `ninfer-serve` automatically adds an owned `--request-log-jsonl` path unless the user supplied one. NInfer does not expose Prometheus; its direct `ninfer_*` metrics are collected by the NInfer JSONL recorder.
